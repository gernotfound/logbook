...
const App = {
    ...
    showAppStatus() {
        const isOnline = navigator.onLine ? "Online ­ƒƒó" : "Offline ­ƒö┤";
        ...
        this.tempNewExMuscles.forEach(mId => {
            ...
            if(m) {
                html += `<span style="background:var(--primary-color); color:#fff; padding:4px 8px; border-radius:12px; font-size:0.8rem; display:flex; align-items:center; gap:5px;">
                    ${m.name} <span style="cursor:pointer; font-weight:bold;" onclick="App.removeMuscleFromNewEx('${m.id}')">├ù</span>
                </span>`;
            }
        });
    ...
    createExerciseInLibrary() {
        ...
        if(this.editingExerciseId) {
            if(this.state.library.some(e => e.id !== this.editingExerciseId && e.name.toLowerCase() === name.toLowerCase())) {
                return alert("Nome esercizio gi├á esistente!");
            }
            ...
            if(this.state.library.some(e => e.name.toLowerCase() === name.toLowerCase())) {
                return alert("Esercizio gi├á esistente!");
            }
    ...
    deleteLibraryEx(id) {
        if(confirm("Eliminare questo esercizio? Verr├á mostrato come 'Rimosso' nelle vecchie schede.")){
        ...
        this.state.library.forEach(ex => {
            html += `<div class="card" style="padding:15px; margin-bottom:15px;">
                <div style="display:flex; justify-content:space-between; margin-bottom:10px;">
                    ...
                    <div>
                        <button class="btn-icon" style="color:var(--warning-color); margin-right: 5px;" onclick="App.editLibraryEx('${ex.id}')">Ô£Å´©Å</button>
                        <button class="btn-icon" style="color:var(--danger-color)" onclick="App.deleteLibraryEx('${ex.id}')">­ƒùæ</button>
                    </div>
                </div>
    ...
    createRoutine() {
        ...
        if(this.state.routines.some(r => r.name.toLowerCase() === name.toLowerCase())) return alert("Nome esiste gi├á!");
    ...
    updateRoutineName(id, value) {
        ...
        if(this.state.routines.some(x => x.id !== id && x.name.toLowerCase() === value.trim().toLowerCase())) {
            alert("Nome gi├á in uso."); return this.renderRoutineBuilder();
        }
        r.name = value.trim();
        ...
        this.state.routines.forEach(r => {
            html += `<div class="card" style="margin-bottom:20px;">
                <div style="display:flex; justify-content:space-between; margin-bottom:15px;">
                    ...
                    <button class="btn-icon" style="color:var(--danger-color);" onclick="App.deleteRoutine('${r.id}')">­ƒùæ</button>
                </div>
            ...
            r.exercises.forEach((re, idx) => {
                ...
                html += `<div style="display:flex; align-items:center; justify-content:space-between; font-size:0.9rem; padding:10px 0; border-bottom:1px solid var(--glass-border);">
                    ...
                    <div style="display:flex; gap:2px; margin-left:10px;">
                        <button class="btn-icon" onclick="App.moveExInRoutine('${r.id}', ${idx}, -1)">Ô¼å´©Å</button>
                        <button class="btn-icon" onclick="App.moveExInRoutine('${r.id}', ${idx}, 1)">Ô¼ç´©Å</button>
                        <button class="btn-icon" style="color:var(--danger-color);" onclick="App.removeExFromRoutine('${r.id}', ${idx})">ÔØî</button>
                    </div>
                </div>`;
            });
            html += `</div>
    ...
    startWorkout(routineId) {
        if(this.state.activeWorkout) return alert("Hai gi├á un allenamento in corso!");
            ...
            html += `<div style="margin-bottom:25px; padding-bottom:20px; border-bottom:1px solid rgba(255,255,255,0.1);">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
                    ...
                    <div style="display:flex; gap:5px;">
                        <button class="btn-small" style="background:rgba(239, 68, 68, 0.1); border:1px solid var(--danger-color); color:var(--danger-color); border-radius:8px;" onclick="App.removeActiveExercise(${exIndex})">­ƒùæ</button>
                        <button class="btn-small toggle-btn" onclick="App.toggleUI('hist-${exItem.exId}-${exIndex}', this)">­ƒôè Storico</button>
                        <button class="btn-small toggle-btn" onclick="App.toggleUI('setup-${exItem.exId}-${exIndex}', this)">­ƒôØ Setup</button>
                    </div>
                </div>
                ...
                ${lastNote ? `<div style="background:rgba(239, 68, 68, 0.1); padding:10px; border-radius:8px; border-left:3px solid var(--danger-color); font-size:0.85rem; margin-bottom:15px; color:#fca5a5;">ÔÜá´©Å <b>Note scorsa volta:</b> ${lastNote}</div>` : ''}
                `;
            exItem.sets.forEach((s, sIndex) => {
                ...
                if(s.dropsets) {
                    s.dropsets.forEach((ds, dsIdx) => {
                        specialHtml += `<div class="set-row special-row" style="margin-left: 20px; border-left: 2px solid var(--warning-color); padding-left: 10px;">
                            <div style="font-size:0.75rem; color:var(--warning-color);">Ôå│ Dropset</div>
                            <div class="set-controls">
                                ...
                                <button class="btn-icon" style="color:var(--danger-color); padding:0 5px;" onclick="App.removeSpecialSet(${exIndex}, '${s.id}', 'dropsets', ${dsIdx})">├ù</button>
                            </div>
                        </div>`;
                    });
                }
                if(s.isometrics) {
                    s.isometrics.forEach((iso, isoIdx) => {
                        specialHtml += `<div class="set-row special-row" style="margin-left: 20px; border-left: 2px solid var(--accent-color); padding-left: 10px;">
                            <div style="font-size:0.75rem; color:var(--accent-color);">Ôå│ Isometria</div>
                            <div class="set-controls">
                                ...
                                <button class="btn-icon" style="color:var(--danger-color); padding:0 5px;" onclick="App.removeSpecialSet(${exIndex}, '${s.id}', 'isometrics', ${isoIdx})">├ù</button>
                            </div>
                        </div>`;
                    });
                }
                ...
                <div class="set-row">
                    <div style="display:flex; align-items:center; gap:8px;">
                        ...
                        <button class="btn-icon" style="color:var(--danger-color); font-size:1rem;" onclick="App.removeActiveSet(${exIndex}, ${sIndex})">­ƒùæ</button>
                    </div>
                    <div class="set-controls">
    ...
    renderRoutinesList() {
        ...
        filtered.forEach(r => {
            ...
            const exSummary = exSummaryList.length > 0 ? exSummaryList.join(' ÔÇó ') : 'Nessun esercizio';
            html += `<div class="routine-card">
                ...
                <button class="btn btn-primary" style="width:100%; margin-top:10px; margin-bottom:0;" onclick="App.startWorkout('${r.id}')">ÔûÂ´©Å Inizia ${r.name}</button>
            </div>`;
        });
        container.innerHTML = html;
        filtered.forEach(r => {
        ...
        historyItems.forEach(w => {
            ...
            html += `<div class="card" style="border-left: 4px solid var(--primary-dark);">
                <div style="display:flex; justify-content:space-between; margin-bottom:15px;">
                    ...
                    <button class="btn-icon" style="color:var(--danger-color);" onclick="App.deleteHistoryRecord('${w.id}')">­ƒùæ</button>
                </div>
                <div style="margin-bottom:15px; font-size:0.9rem; color:var(--text-muted)">
                    ...
                    ${w.waterLiters ? `<span style="margin-left: 15px; color:var(--primary-color);">­ƒÆº Acqua: ${w.waterLiters}L</span>` : ''}
                </div>
                ${ratingsHtml}`;
    ...
    renderNutritionDisplay() {
        ...
        mealCategories.forEach(mName => {
            ...
            if (subtotalElem) {
                subtotalElem.innerText = `${mealTotals.kcal} kcal ÔÇó C:${mealTotals.carbs} P:${mealTotals.pro} F:${mealTotals.fat}`;
            }
                        ...
                        html += `
                            <div class="logged-food-item">
                                <div class="logged-food-info">
                                    ...
                                    <div class="logged-food-macros">­ƒöÑ ${item.kcal} kcal | C: ${item.carbs}g | P: ${item.pro}g | F: ${item.fat}g</div>
                                </div>
                                <div class="logged-food-actions">
                                    <button class="chip-btn" onclick="App.openEditMealItemModal('${mName}', '${item.id}')" title="Modifica">Ô£Å´©Å</button>
                                    <button class="chip-btn" style="color:#ef4444;" onclick="App.deleteMealItem('${mName}', '${item.id}')" title="Elimina">­ƒùæ</button>
                                </div>
                            </div>
                        `;
                    });
                ...
                sortedDates.forEach(d => {
                    ...
                    histHtml += `<div class="card" style="padding:15px; margin-bottom:15px;">
                        <div style="display:flex; justify-content:space-between; margin-bottom:10px;">
                            ...
                            <button class="btn-icon" style="color:var(--danger-color)" onclick="App.deleteNutritionDay('${d}')">­ƒùæ</button>
                        </div>
                ...
                html += `
                    <div class="food-result-item" onclick="App.selectSearchFood('${food.id}')">
                        <div>
                            ...
                            <div style="font-size:0.75rem; color:var(--text-muted);">${food.category || 'Alimento'} ÔÇó ${servingInfo}</div>
                        </div>
    ...
    openQtyModal(food, existingItem = null, targetMealName = null) {
        ...
        if (confirmBtn) {
            confirmBtn.innerText = existingItem ? '­ƒÆ¥ Aggiorna Pasto' : 'Ô×ò Aggiungi al Pasto';
        }
    ...
    confirmAddFoodToMeal() {
        ...
        const unit = unitSelect ? unitSelect.value : (this._activeQtyFood.unit || 'g');
        if (qty <= 0) {
            alert("Inserisci una quantit├á valida maggiore di 0");
            return;
        }
        const nutrients = Logic.scaleFoodNutrients(this._activeQtyFood, qty, unit);
    ...
    async saveMeasurement(event) {
        ...
        if (!this.state.nutrition[dateStr]) {
            ...
            if (!confirm(`Esiste gi├á una misurazione per la data ${dateStr}. Vuoi sovrascriverla?`)) {
                return;
            }
        }
        const dayData = this.state.nutrition[dateStr];
            ...
            html += `
                <div class="measurement-card">
                    <div class="measurement-header">
                        ...
                        <div>
                            <button class="chip-btn" onclick="App.editMeasurement('${d}')" title="Modifica">Ô£Å´©Å</button>
                            <button class="chip-btn" style="color:var(--danger-color);" onclick="App.deleteMeasurement('${d}')" title="Elimina">­ƒùæ</button>
                        </div>
                    </div>
    ...
    renderHomeDashboard() {
        ...
        document.getElementById('home-streak-display').innerText = "­ƒöÑ " + this.state.history.length;
