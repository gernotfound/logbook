...
## 19. Testing E2E e prevenzione errori su GitHub Actions
- **Doppia suite obbligatoria:** Lanciare sempre *entrambe* le suite di test in locale (`npm run build && npm run test && npx playwright test`) come ultimissimo step, solo **dopo** aver completato qualsiasi modifica a componenti visuali e **prima** di fare `git push`. Questo previene fallimenti nei workflow automatizzati.
- **Ancore per bot di test (Playwright):** Se si vanno a modificare testi, pulsanti o placeholder dell'interfaccia (che fungono da "ancore" per i bot di test), � **tassativo** prestare attenzione e verificare se quei testi compaiono nei file `e2e/*.spec.ts`. In tal caso, i test E2E vanno aggiornati di conseguenza prima del commit.
