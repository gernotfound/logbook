export const Logic = {
    ...
    validateCustomFood(foodData) {
        ...
        if (!['g', 'ml', '1pezzo', 'pezzo', 'pcs'].includes(unit)) {
            errors.unit = "Unit├á di misura non valida (usare g, ml o pezzo)";
        }
        const baseQty = parseFloat(foodData.baseQty) || 100;
        if (baseQty <= 0) {
            errors.baseQty = "La quantit├á base deve essere maggiore di 0";
        }
        ...
        if (satFat + unSatFat > fat + 0.01) {
            errors.satFat = "La somma dei grassi saturi e insaturi non pu├▓ superare i grassi totali";
        }
        const isValid = Object.keys(errors).length === 0;
