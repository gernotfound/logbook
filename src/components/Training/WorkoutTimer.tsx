...
function OwnerWorkoutTimer({ owner }: { owner: string }) {
    ...
    // The interval only schedules refreshes: elapsed time always comes from Date.now(),
    // so background throttling cannot make the displayed timer drift.
    useEffect(() => {
    ...
    return (
        <div className="workout-rest-timer" data-state={restState}>
            <span className="workout-rest-timer__display" aria-live="off" aria-label={`Timer recupero ${restDisplay}`}>
                {restDisplay}
            </span>
