        ...
        <React.Fragment>
                ...
                <div style={{ display: 'flex', gap: '5px', flex: 1, position: 'relative' }}>
                    ...
                    {isOpenMenu && (
                        <div className="special-menu" style={{ position: 'absolute', right: 0, top: '35px', background: 'var(--surface-color)', padding: '10px', borderRadius: '8px', zIndex: 10, boxShadow: '0 4px 12px rgba(0,0,0,0.5)', border: '1px solid var(--glass-border)' }}>
                            <button className="btn btn-small" style={{ display: 'block', width: '100%', marginBottom: '5px' }} onClick={() => onAddSpecialSet('dropset')}>+ Dropset</button>
            ...
            {(s.dropsets || []).map((ds: any, dsIdx: number) => (
                <div key={ds.id || dsIdx} style={{ marginLeft: '20px', borderLeft: '2px solid var(--warning-color)', paddingLeft: '10px', display: 'flex', alignItems: 'center', marginBottom: '5px', gap: '10px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '5px', flex: 1 }}>
                        <input id={`ds-kg-${s.id}-${dsIdx}`} type="number" step="0.25" placeholder="Kg" value={ds.kg} onChange={e => onUpdateSpecialSet('dropsets', dsIdx, 'kg', e.target.value)} style={{ margin: 0, flex: 1 }} />
                        <input id={`ds-reps-${s.id}-${dsIdx}`} type="number" placeholder="Reps" value={ds.reps} onChange={e => onUpdateSpecialSet('dropsets', dsIdx, 'reps', e.target.value)} style={{ margin: 0, flex: 1 }} />
            ...
            {(s.isometrics || []).map((iso: any, isoIdx: number) => (
                <div key={iso.id || isoIdx} style={{ marginLeft: '20px', borderLeft: '2px solid var(--accent-color)', paddingLeft: '10px', display: 'flex', alignItems: 'center', marginBottom: '5px', gap: '10px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '5px', flex: 1 }}>
                        <input id={`iso-kg-${s.id}-${isoIdx}`} type="number" step="0.25" placeholder="Kg" value={iso.kg} onChange={e => onUpdateSpecialSet('isometrics', isoIdx, 'kg', e.target.value)} style={{ margin: 0, flex: 1 }} />
                        <input id={`iso-time-${s.id}-${isoIdx}`} type="number" placeholder="Sec" value={iso.time} onChange={e => onUpdateSpecialSet('isometrics', isoIdx, 'time', e.target.value)} style={{ margin: 0, flex: 1 }} />
