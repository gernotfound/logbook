...
const HomeView = ({ onNavigate }) => {
  ...
  const todayNutrition = nutrition[todayStr] || { kcal: 0, carbs: 0, pro: 0, fat: 0, weight: 0 };
  ...
  // Get Targets
  const kcalTarget = userData?.nutritionPlanning?.normocalorica?.kcal || 2500;
  
  // Get BF %
  const currentWeight = todayNutrition.weight || (Object.values(nutrition).reverse().find(n => n.weight)?.weight) || userData?.nutritionPlanning?.weight || 80;
  let bf = "--";
    ...
    <div id="view-home" className="view-section active">
      ...
      <div className="card" id="home-nutrition-widget">
          ...
          <div className="progress-bg" style={{ marginBottom: '20px' }}>
              <div className="progress-fill" style={{ width: `${Math.min((kcalEaten/kcalTarget)*100, 100)}%`, background: 'linear-gradient(90deg, var(--warning-color), #fcd34d)' }}></div>
          </div>
      ...
      <div style={{ display: 'flex', gap: '15px', marginBottom: '20px' }}>
          ...
          <div className="card" style={{ flex: 1, textAlign: 'center', marginBottom: 0, padding: '15px' }}>
              <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Workout Totali</div>
              <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--text-main)', marginTop: '5px' }}>🔥 {streak}</div>
          </div>
      </div>
      ...
      <div className="card" id="home-chart-widget">
          ...
          <div style={{ position: 'relative', height: '200px', width: '100%' }}>
              {recentDates.length > 0 ? (
                  ...
                  <p style={{textAlign:'center', color:'var(--text-muted)', paddingTop:'80px'}}>Nessun dato sul peso.</p>
              )}
          </div>
      </div>
    </div>
