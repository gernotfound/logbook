...
export async function migrateGuestAccount({
    ...
    const assertCurrent = () => {
        if (!isCurrent()) throw new StaleGuestMigrationError();
    };
    ...
    try {
        ...
        if (guestHasData) {
            ...
            // The authenticated envelope is now durable. Only a still-current auth run may
            // retire guest ownership and start replication for this UID.
            markLocalReady();
