...
export class TelemetryHub {
  ...
  public trackError(error: unknown, options: TrackErrorOptions = {}): void {
    try {
      ...
      if (options.customMessage) {
        fastMsg = options.customMessage;
      } else if (error instanceof Error) {
        fastType = error.name || 'Error';
        fastMsg = error.message || '';
      } else if (typeof error === 'string') {
        fastMsg = error;
      } else if (typeof error === 'number' || typeof error === 'boolean') {
        fastMsg = String(error);
      } else if (error === null) {
        fastMsg = 'null';
      ...
      if (error instanceof Error && !options.customMessage && !('issues' in (error as any))) {
  ...
  private saveQueuedEvents(items: QueuedTelemetryItem[]): void {
    try {
      if (typeof localStorage !== 'undefined') {
  ...
  public async flushQueue(): Promise<void> {
    try {
      ...
      for (const item of items) {
        ...
        if (typeof item.payload === 'object' && (!item.payload.userId || item.payload.userId === 'anonymous')) {
      ...
      const remaining = currentLiveQueue.filter(
        (q) => q && q.id && !successfullyDispatchedIds.has(q.id) && q.payload
      );
      this.saveQueuedEvents(remaining);
    } catch (err) {
  ...
  public async dispatchErrorToFirestore(payload: TelemetryErrorPayload): Promise<boolean> {
    try {
      ...
      const docRef = doc(db, 'users', uid, 'telemetry_errors', errorId);
  ...
  public async dispatchEventToFirestore(payload: TelemetryEventPayload): Promise<boolean> {
    try {
      ...
      const docRef = doc(db, 'users', uid, 'telemetry_events', eventId);
